 ...
 * Si occupa di encryption e decryption con RSA2048 (PKCS 1.5/2.1, non OAEP)
 * Funziona con file e stringhe
 * E' lento e non più usato. Conservato per legacy.
 * 
 * Nel nuovo service facciamo solo decryption (encryption fatta client-side)
 * 
 */
...
namespace DoubleEncryption.Services
{
    ...
    {
        public int _keyLength = 2048;
        public int _blockSize = 256;
        public int _chunkSize = 245;
        //public int _keyLength = 1024;
        //public int _blockSize = 128;
        //public int _chunkSize = 117;
        public string _gs = "|";
        public string _gsRegex = @"\|";

        public KeyPair handshake()
        {
            RSACryptoServiceProvider rsa = new RSACryptoServiceProvider(_keyLength);
            //KeyPair chiaviXML = new RSAKeyPair()
            //{
            //    publicKey = rsa.ToXmlString(false),
            //    privateKey = rsa.ToXmlString(true)
            //};
            //KeyPair chiaviPEM = new RSAKeyPair()
            //{
            //    publicKey = PublicXML2PEM(rsa),
            //    privateKey = PrivateXML2PEM(rsa)
            //};

            KeyPair chiavi = new KeyPair()
            {
                publicKey = PublicXML2PEM(rsa),
                privateKey = rsa.ToXmlString(true)
            };
        ...
        {
            var strOutput = "";
            var stringParts = Split(text, _chunkSize);
            foreach (var stringPart in stringParts)
            {
                strOutput += encrypt(publicKey, stringPart, pem) + _gs;
            }
            return strOutput;
        }
